...
var COFantasy = COFantasy || function() {
      ...
      cibles = cibles.filter(function(target) {
        ...
        if (options.auto) {
          ...
          if (d20roll == 1 && options.chance === undefined) {
            ...
            attackResult = " : <span style='"+bs_label+" "+bs_label_success+"'><b>réussite</b></span>";
            touche = 1;
            champion = true;
          } else if (attackRoll < defense) {
            attackResult = " : <span style='"+bs_label+" "+bs_label_warning+"'>échec</span>";
            touche = false;
          } else { // Touché normal
            attackResult = " : <span style='"+bs_label+" "+bs_label_success+"'><b>réussite</b></span>";
            touche = 1;
  ...
  function attaqueMagique(msg, evt, callback) {
    ...
    sendChat("", toEvaluate, function(res) {
      ...
      var titre =
        ...
        "</b>";
      var display = startFramedDisplay(msg.playerid, titre, char1, token2);
      var line =
  ...
  function distribuerBaies(msg) {
    ...
    var niveau = charAttributeAsInt(charIdDruide, 'NIVEAU', 1);
    var evt = {
