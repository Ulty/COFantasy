...
var COFantasy = COFantasy || function() {
      ...
      switch (cmd[0]) {
        ...
        case "effet":
          ...
          if (estEffetTemp(cmd[1])) {
            ...
            options.seulementVivant = messageEffetTemp[cmd[1]] && messageEffetTemp[cmd[1]].seulementVivant;
  ...
  function iterTokensOfEffet(charId, pageId, effet, attrName, foo, filterUnique) {
  ...
  function nextTurnOfActive(active, attrs, evt, pageId) {
    if (active.id == "-1" && active.custom == "Tour") {
      ...
      attrsSave.forEach(function(attr) {
        ...
        var token;
        iterTokensOfEffet(charId, pageId, effet, attrName, function(tok) {
